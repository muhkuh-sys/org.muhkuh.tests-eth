...
int eth_send(ETH_INFO_T* ptEthInfo, const void* pvData, unsigned int uiLen, ETH_TC_E ePrio, unsigned int uTxTstampBuf, ETH_TX_CNF_HANDLER_T* ptCnfHandler)
{
  ...
  memcpy((void*)&tHdr.tUntagged.tDstAddr, ptEthInfo->ptDstAddr, sizeof(tHdr.tUntagged.tDstAddr));
  memcpy((void*)&tHdr.tUntagged.tSrcAddr, ptEthInfo->ptSrcAddr, sizeof(tHdr.tUntagged.tSrcAddr));
