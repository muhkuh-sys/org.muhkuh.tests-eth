...
int NX5_EXTPHY_GetLinkState( unsigned int uPortNo, unsigned int* puLink, unsigned int* puMbps, unsigned int* puFullDuplex, void* pvUser )
{
  ...
  /* read out PHY status */
  NX5_MIIMU_ReadPhyReg(NX5_MIIMU_SEND_PREAMBLE, NX5_MIIMU_MDCFREQ_HIGH, NX5_MIIMU_RTA_0_BITS, uPortNo + NX5_EXT_PHY1_ADDR, BCM5241_AUXILIARY_STATUS_SUM, &usMiiData, NULL);
...
int NX5_XC_Reset( unsigned int uiPort, void* pvUser )
{
  ...
  pulPram = s_pulXpecPramStart[uiPort];
  pulDram = s_pulXpecDramStart[uiPort];
  ...
  NX_WRITE32(ptXpec->aulStatcfg[uiPort], 0);

  /* load ratemul reset code */
  if( NX5_XC_Load(uiPort, NX5_XC_TYPE_RPU, (unsigned long*)paulxMacRpuCodes[uiPort], pvUser)!=0 ) {
    return -1;
  }
  if( NX5_XC_Load(uiPort, NX5_XC_TYPE_TPU, (unsigned long*)paulxMacTpuCodes[uiPort], pvUser)!=0 ) {
    return -1;
  }
...
int NX5_XC_Load( unsigned int uiPort, NX5_XC_TYPE_E eXcType, unsigned long* pulXcPrg, void* pvUser )
{
  ...
  /* PRAM Area */
  pulRange1Dpm    = s_pulXpecPramStart[uiPort];
  ulRange1Start   = aulNetx5XpecPram[uiPort];
  ulRange1Size    = NX5_XPEC_DWORD_RAMSIZE * sizeof(unsigned long);

  /* DRAM Area */
  pulRange2Dpm    = s_pulXpecDramStart[uiPort];
  ulRange2Start   = aulNetx5XpecDram[uiPort];
  ...
  switch( eXcType ) {
    ...
    case NX5_XC_TYPE_XPEC:
      ...
      pulPRamStart    = s_pulXpecPramStart[uiPort];
      pulPRamEnd      = pulPRamStart + NX5_XPEC_DWORD_RAMSIZE;

      /* Register Area */
      pulRange0Dpm    = (volatile uint32_t*)s_ptXpecStart[uiPort];
      ulRange0Start   = aulNetx5XpecReg[uiPort];
      ulRange0Size    = 0x0104;
      break;

    case NX5_XC_TYPE_RPU:
      /* Pointer to PRAM area */
      pulPRamStart    = s_ptXmacStart[uiPort]->aulXmac_rpu_program;
      pulPRamEnd      = pulPRamStart + NX5_XMAC_RPU_DWORD_RAMSIZE;

      /* PRAM Area */
      pulRange0Dpm    = (volatile uint32_t*)s_ptXmacStart[uiPort];
      ulRange0Start   = aulNetx5Xmac[uiPort];
      ulRange0Size    = 0x0a08;
      break;

    case NX5_XC_TYPE_TPU:
      /* Pointer to PRAM area */
      pulPRamStart    = s_ptXmacStart[uiPort]->aulXmac_tpu_program;
...
*   -1 on erroneous                                                          */
/*****************************************************************************/
int NX5_XC_StartUnits(unsigned int uiPort, unsigned int uiUnitVec, void* pvUser)
{
  NX5_XMAC_AREA_T* ptXmac;
  NX5_XPEC_AREA_T* ptXpec;
  ...
  if( 0 != (uiUnitVec & MSK_NX5_XC_TYPE_RPU) )
  {
    NX_WRITE32(ptXmac->ulXmac_rpu_hold_pc, 0);
  }

  if( 0 != (uiUnitVec & MSK_NX5_XC_TYPE_TPU) )
  {
    NX_WRITE32(ptXmac->ulXmac_tpu_hold_pc, 0);
  }

  if( 0 != (uiUnitVec & MSK_NX5_XC_TYPE_XPEC) )
  {
    NX_WRITE32(ptXpec->ulXpu_hold_pc, 0);
  }
