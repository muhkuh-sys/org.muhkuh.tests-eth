...
*  -1                                                                        */
/*****************************************************************************/
int MLEDCTRL_ConfigureOutput(unsigned int uiInst,
                             ...
                             unsigned int uiBrightness)
{
  ...
  /* set output select */
  ulVal = (MLED_CTRL_SEL_PASSTHROUGH == eOutputSel) ?  HW_MSK(mled_ctrl_output_sel0_sel) : (uint32_t)eOutputSel << HW_SRT(mled_ctrl_output_sel0_sel);
...
* \return                                                                    */
/*****************************************************************************/
void MLEDCTRL_Start (unsigned int uiInst, unsigned int uiPrescaleCounterMax, unsigned int uiBlinkCounterMax)
{
...
* \return                                                                    */
/*****************************************************************************/
void MLEDCTRL_Stop (unsigned int uiInst)
{
...
* \return                                                                    */
/*****************************************************************************/
void MLEDCTRL_SetLine (unsigned int uiInst, uint32_t ulVal)
{
...
/*! MLEDCTRL Set led
* \description
*   This function activates/deactivates an individual MLED output. Only works
*   when "line mode" is selected for corresponding output.
* \class
*   MLEDCTRL
* \params
*   uiInst     [in] Instance
*   uiOutput   [in] MLED output
*   fVal       [in] Activate (true) or deactivate (false)
* \return                                                                    */
/*****************************************************************************/
void MLEDCTRL_SetOutput (unsigned int uiInst, unsigned int uiOutput, bool fVal)
{
  if(uiOutput > 7) {
    return;
  }
  if(fVal) {
    s_ptMledCtrl->ulMled_ctrl_line0 |= (1U << uiOutput);
  }
  else {
    s_ptMledCtrl->ulMled_ctrl_line0 &= ~(1U << uiOutput);
  }
