...
*                                                                            */
/*****************************************************************************/
void NX10_UART_InitSerial( unsigned int uiPort )
{
  unsigned int ulUartBaseAdr;

  // get uart base adr
  ulUartBaseAdr = aNX10UartBaseAdr[uiPort];
...
*                                                                            */
/*****************************************************************************/
void NX10_UART_CloseSerial(unsigned int uiPort)
{
   unsigned int ulUartBaseAdr;
   unsigned int uCnt;
   ...
   for(uCnt=0;uCnt<5;uCnt++) {
     PEEK(ulUartBaseAdr+REL_Adr_NX10_uartdr); /* Read out the reception register if there are still bytes in it */
   }
}
...
*                                                                            */
/*****************************************************************************/
int NX10_UART_PutCharacter(unsigned int uiPort, int iChar)
{
  unsigned int ulUartBaseAdr;
  ...
  POKE( ulUartBaseAdr+REL_Adr_NX10_uartdr, iChar);
...
*   -1 on erroneous                                                          */
/*****************************************************************************/
int NX10_UART_GetCharacter(unsigned int uiPort)
{
  unsigned int uDat;
  unsigned int ulUartBaseAdr;
...
*   1                                                                        */
/*****************************************************************************/
int NX10_UART_Peek(unsigned int uiPort)
{
  unsigned int ulUartBaseAdr;
...
*   GPIO
* \params
*   ulCounter          [in]  Selected GPIO timer
* \return
*                                                                            */
/*****************************************************************************/
void NX10_GPIO_ResetTimer( unsigned long ulCounter )
{
  s_ptGpio->aulGpio_counter_ctrl[ulCounter] = 0;
  s_ptGpio->aulGpio_counter_cnt[ulCounter]  = 0;
  s_ptGpio->aulGpio_counter_max[ulCounter]  = 0;
  s_ptGpio->ulCnt_irq_mask_rst             = 1 << ulCounter;
  s_ptGpio->ulCnt_irq_raw                  = 1 << ulCounter;
}
...
void NX10_MMIO_SetMode( unsigned long ulMmioNum, unsigned long ulMode )
{
  if( NX10_MMIO_MODE_OUT == ulMode)
    s_ptMmioCtrl->ulMmio_pio_oe_line_cfg |= 1UL << ulMmioNum;
  else
    s_ptMmioCtrl->ulMmio_pio_oe_line_cfg &= ~(1UL << ulMmioNum);
}
...
{
  if(fEnable) {
    s_ptMmioCtrl->ulMmio_pio_out_line_cfg |= 1UL << ulMmioNum;
  } else {
    s_ptMmioCtrl->ulMmio_pio_out_line_cfg &= ~(1UL << ulMmioNum);
  }
}
...
void NX10_INTPHY_Init( NX10_PHY_CONTROL_T *ptPhyCtrlInit )
{
  ...
  {
    NX10_INTPHY_CorrectAutoMdix100MBit(uPhyAdr);
  }
...
*   PFIFO
* \params
*   auiPFifoDepth  [in]  Array of 16 Elements containing the depth of each FIFO
* \return
*                                                                            */
/*****************************************************************************/
int NX10_PFIFO_SetBorders(const unsigned int *auiPFifoDepth)
{
  int iResult;
  ...
  s_ptPFifo->ulPfifo_reset = 0xffff;

  /* define pointer fifo borders */
  uiBorder = 0;
  for(uiFifoNum = 0; uiFifoNum < 16; uiFifoNum++)
  {
...
*   PFIFO
* \params
*   auiPFifoDepth  [out] Array of 16 Elements containing the depth of each FIFO
* \return
*                                                                            */
/*****************************************************************************/
int NX10_PFIFO_GetBorders(unsigned int *auiPFifoDepth)
{
  int iResult;
...
int NX10_HSCTRL_AreHsFlagsEqual( unsigned int uInst, unsigned long ulHsFlagNum1, unsigned long ulHsFlagNum2 )
{
  ...
  if( ((ulVal>>ulHsFlagNum1)&1)==((ulVal>>ulHsFlagNum2)&1) ) return 1; else return 0;
}
...
*   -1 on erroneous                                                          */
/*****************************************************************************/
int NX10_XC_Reset( unsigned int uiPort, void* pvUser )
{
  ...
  if( uiPort >= NX10_NUM_XPECS)
    return -1;

  ptXmac      = s_ptXmacArea[uiPort];
  ptXpec      = s_ptXpecRegArea[uiPort];
  pulXpecPram = s_pulXpecPramArea[uiPort];
  pulXpecDram = s_pulXpecDramArea[uiPort];
  ...
  if( NX10_XC_Load( uiPort, NX10_XC_TYPE_RPU, (unsigned long*)paulxMacRpuCodes[uiPort], pvUser)!=0 ) {
    return -1;
  }
  if( NX10_XC_Load( uiPort, NX10_XC_TYPE_TPU, (unsigned long*)paulxMacTpuCodes[uiPort], pvUser)!=0 ) {
    return -1;
  }
...
*   -1 on erroneous                                                          */
/*****************************************************************************/
int NX10_XC_Load(unsigned int uiPort, NX10_XC_TYPE_E eXcType, const unsigned long* pulXcPrg, void* pvUser)
{
  ...
  if(uiPort >= NX10_NUM_XPECS) return -1;

  /* get the start and end address of the ram area, get the physical address */
  switch( eXcType )
  {
    case NX10_XC_TYPE_XPEC: pulRamStart     = (volatile unsigned long*) s_ptXpecRegArea[uiPort];
                            pulRamEnd       = pulRamStart + (sizeof(NX10_XPEC_AREA_T) / sizeof(unsigned long));
                            ulPhysicalStart = (unsigned long) s_ptXpecRegArea[uiPort];
                            break;
    case NX10_XC_TYPE_RPU:  /* fall through */
    case NX10_XC_TYPE_TPU:  pulRamStart     = (volatile unsigned long*)s_ptXmacArea[uiPort];
                            pulRamEnd       = pulRamStart + (sizeof(NX10_XMAC_AREA_T)/sizeof(unsigned long));
                            ulPhysicalStart = (unsigned long) s_ptXmacArea[uiPort];
                            break;
...
*   -1 on erroneous                                                          */
/*****************************************************************************/
int NX10_XC_Start(unsigned int uiPort, void* pvUser)
{
  NX10_XMAC_AREA_T* ptXmac;
  NX10_XPEC_AREA_T* ptXpec;
...
*   -1 on erroneous                                                          */
/*****************************************************************************/
int NX10_XC_StartUnits(unsigned int uiPort, unsigned int uiUnitVec, void* pvUser)
{
  NX10_XMAC_AREA_T* ptXmac;
  NX10_XPEC_AREA_T* ptXpec;
  ...
  if( 0 != (uiUnitVec & MSK_NX10_XC_TYPE_RPU) )
  {
    ptXmac->ulXmac_rpu_hold_pc = 0;
  }

  if( 0 != (uiUnitVec & MSK_NX10_XC_TYPE_TPU) )
  {
    ptXmac->ulXmac_tpu_hold_pc = 0;
  }

  if( 0 != (uiUnitVec & MSK_NX10_XC_TYPE_XPEC) )
  {
    ptXpec->ulXpu_hold_pc      = 0;
  }
