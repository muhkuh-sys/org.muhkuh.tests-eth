...
#define SERIAL_PUT(x)  s_tStdout.put(x, s_tStdout.pvStream)
#define SERIAL_FLUSH() if(s_tStdout.flush)s_tStdout.flush(s_tStdout.pvStream)
...
int uprint_putchar_dummy(int i, void* pvInst){ return -1; }
static UPRINT_STREAM_T s_tStdout = { uprint_putchar_dummy, NULL, NULL };
static CONSOLE_LINEFEED_T s_tLinefeedMode = CONSOLE_LINEFEED_CRLF;

void uprint_init(uprint_putc_fn put, uprint_flush_fn flush, void* pvStream)
{
  s_tStdout.put = put;
  s_tStdout.flush = flush;
  s_tStdout.pvStream = pvStream;
}
