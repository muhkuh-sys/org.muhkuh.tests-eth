...
def configure(conf):
    ...
    # check if netx90_app_ifölash_image.py is available
    p = os.path.abspath( os.path.join(os.path.dirname(__file__), 'hboot_image_compiler/netx90_app_iflash_image.py'))
    if not os.path.isfile(p):
        p = ''

    conf.msg("Checking for appflash image update tool", p or False)
...
def get_tgen_by_name(self, name):
    """
        ...
        This function has been extended by Hilshcer to support internal task generator mappings:

        Imagine a target name defined as "arm-none-eabi-gcc/4.5.2/protocol-lib" where
        "arm-none-eabi-gcc/4.5.2" depends on the toolchain used to compile the library you can either use
        "arm-none-eabi-gcc/4.5.2/protocol-lib" to refer to this target or use
        "arm-none-eabi-gcc/*/protocol-lib". The latter use will match independently of the compiler version
        number. This allows to link binary distribution libraries with different compiler versions without updating
        the use references all the time.
    """
    default_cache = self.task_gen_cache_names
                ...
                try:
                    ...
                    if m:
                        ...
                        if alias not in aliases:
                            ...
                            if alias in hilscher_cache:
                                tg_old = hilscher_cache[n]
                                old_script = tg_old.path.find_node('wscript')
                                Logs.warn('Warning: Alias "%s" already used for task generator "%s" defined in "%s"' % (alias, tg_old.name, old_script.nice_path()))
                            else:
    ...
    try:
        ...
        cur_script = self.cur_script or self.path.find_node('wscript')

        known_str = ''
        # difflib might be not available when running waf, in that case, do not abort
        # with import error.
        try:
            ...
            # Use the difflibs SequenceMatcher to calculate a value which repesents
            # the similarity of the requested task generator name and the available
            # task generator names. Use that value to order a list and use the first
            # 10 entries of that list to return best known matches
            similar_known_keys = sorted(self.task_gen_hilscher_cache_names.keys(),\
                                        key=lambda x : SequenceMatcher(None,x,name).ratio(),\
                                        reverse=True)[:10]

            if similar_known_keys:
                known_str = '. Did you mean ' + ','.join('%r' % x for x in similar_known_keys) + '?'
...
@feature('disable_gc_sections', 'enable_gc_sections')
def process_gc_sections(self):
    """Supress unknown feature warning """
    pass

@feature('disable_unaligned_access', 'enable_unaligned_access')
def process_unaligned_access(self):
    """Supress unknown feature warning """
    pass
...
def process_source(bld, kw):
    ...
    # enable gc sections by default if not explicitly disabled
    if 'disable_gc_sections' not in features and 'enable_gc_sections' not in features:
        features.append('enable_gc_sections')

    # disable unaligned accesses by default if not explicitly enabled
    if 'disable_unaligned_access' not in features and 'enable_unaligned_access' not in features:
        features.append('disable_unaligned_access')

    # enable c99 by default if not enables
    if not any(x.startswith('standard_c') for x in features):
        features.append('standard_c99')
...
match_prefix = re.compile(r'(?P<triplet>[^/]+)/[0-9]+.[0-9].[0-9]+/').match
...
def handle_toolchain(bld, kw):
    ...
    if m:
        ...
        if name != target_triplet:
            try:
                prefix = bld.get_name_prefix(toolchain = toolchain, platform = platform)
            except:
                pass
            else:
...
def check_tgen_availability(self):
    ...
    tgen_disabled = getattr(self, 'tgen_disabled', False)
    msg = None
...
def task_uid(self):
    ...
    Hilscher extension to this function is to fix umlaut issues due to bad string encoding.
    ...
    try:
        ...
        for x in self.inputs + self.outputs:
            # handling file path encodings properly is real mess
            # the name/abspath function return a string object using the encodig of the
            # file system. In order to avoid issues due to implicit conversions we have to:
            # 1) convert the string object into an unicode object using the file system encoding
            # 2) convert the unicode object into a bytes/string object (depends on python version)
            #    using unicode encoding

            path = x.abspath()

            if not isinstance(path, unicode):
                path = path.decode(sys.getfilesystemencoding()) # Step 1)
            try:
                up(path.encode(encoding='utf-8')) # Step 2)
            except:
                raise
        self.uid_ = m.digest()
    return self.uid_

Task.Task.uid = task_uid
