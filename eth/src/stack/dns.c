...
static const unsigned char *parse_answer(const unsigned char *pucQuery, const unsigned char *pucPacketStart, const unsigned char *pucQueryEnd, unsigned long *pulResultIp)
{
	...
	if( pucQuery!=NULL )
	{
		...
		sizName = (unsigned int)(pucBuffer-aucHostNameBuffer);
		if( sizName==0 )
		{
			/* this is an error */
			pucQuery = NULL;
...
static void dns_recHandler(void *pvData, unsigned int sizDnsLength, void *pvUser __attribute__((unused)))
{
	ETH2_PACKET_T *ptPkt;
