...
#define NXT(id)                  ETHMAC_CONCAT(NX51_,id)
#define MSK_NXT(id)              ETHMAC_CONCAT(MSK_NX51_,id)
#define SRT_NXT(id)              ETHMAC_CONCAT(SRT_NX51_,id)
...
#include "tpu_ethmac0.h"
#include "tpu_ethmac1.h"
#define NXT(id)                  ETHMAC_CONCAT(NX6_,id)
#define MSK_NXT(id)              ETHMAC_CONCAT(MSK_NX51_,id)
#define SRT_NXT(id)              ETHMAC_CONCAT(SRT_NX51_,id)
#endif
                                   ...
                                   void* pvUser )
{
  ...
  if(  ( 0 != pfnXcLoad(uiPort, NX_XC_TYPE_RPU,  paulxMacRpuCodes[uiPort], pvUser) )
     ||( 0 != pfnXcLoad(uiPort, NX_XC_TYPE_TPU,  paulxMacTpuCodes[uiPort], pvUser) )
     ||( 0 != pfnXcLoad(uiPort, NX_XC_TYPE_RPEC, paulRPecCodes[uiPort], pvUser) )
     ||( 0 != pfnXcLoad(uiPort, NX_XC_TYPE_TPEC, paulTPecCodes[uiPort], pvUser) )
    )
    return ETH_ERR_INIT_FAILED;
  ...
  NX_WRITE32(s_aptXpecDRam[uiPort]->tETHMAC_CONFIG_AREA_BASE.ulETHMAC_SYSTIME_BORDER_COPY_PLUS1, NX_READ32(s_ptSystime->ulSystime_border) + 1);

  /* configure PHY LED unit */
  ulActLedFlashPeriod = ulActLedFlashPeriod / 1000000; /* convert from 10ns->10ms */
  switch( ePhyLedCfg ) {
    case ETH_PHY_LED_STATIC:
      NX_WRITE32(s_aptIntPhyCtrl[uiPort]->ulInt_phy_ctrl_led,
