...
*   -1 on erroneous                                                          */
/*****************************************************************************/
int NX6_XC_Reset( unsigned int uiPort,
                  void*        pvUser )
{
  ...
  NX_WRITE32(ptRpec->aulStatcfg[uiPort], 0xffff0000);
  ...
  if( NX6_XC_Load( uiPort, NX6_XC_TYPE_RPU, (uint32_t*)paulxMacRpuCodes[uiPort], pvUser)!=0 ) {
    return -1;
  }
  if( NX6_XC_Load( uiPort, NX6_XC_TYPE_TPU, (uint32_t*)paulxMacTpuCodes[uiPort], pvUser)!=0 ) {
    return -1;
  }
  ...
  NX_WRITE32(s_ptXpecIrqRegs->aulIrq_xpec[uiPort], 0x0000FFFF);
...
*   -1 on erroneous                                                          */
/*****************************************************************************/
int NX6_XC_Load( unsigned int    uiPort,
                 ...
                 void*           pvUser )
{
  ...
  switch( eXcType )
  {
    case NX6_XC_TYPE_RPEC:
      pulRamStart     = s_apulRpecPramArea[uiPort];
      pulRamEnd       = s_apulRpecPramAreaEnd[uiPort]+1;

      /* Register Area */
      pulRange0Dpm    = (volatile uint32_t*)s_aptRpecRegArea[uiPort];
      ulRange0Start   = s_aulNX6RpecRegs[uiPort];
      ulRange0Size    = sizeof(NX51_XPEC_AREA_T);

      /* PRAM Area */
      pulRange1Dpm    = s_apulRpecPramArea[uiPort];
      ulRange1Start   = s_aulNX6RpecPram[uiPort];
      ulRange1Size    = NX6_XPEC_DWORD_RAMSIZE * sizeof(uint32_t);

      /* DRAM Area */
      pulRange2Dpm    = s_apulXpecDramArea[uiPort];
      ulRange2Start   = s_aulNX6XpecDram[uiPort];
      ulRange2Size    = NX6_XPEC_DWORD_RAMSIZE * sizeof(uint32_t);
      break;

    case NX6_XC_TYPE_TPEC:
      pulRamStart     = s_apulTpecPramArea[uiPort];
      pulRamEnd       = s_apulTpecPramAreaEnd[uiPort]+1;

      /* Register Area */
      pulRange0Dpm    = (volatile uint32_t*)s_aptTpecRegArea[uiPort];
      ulRange0Start   = s_aulNX6TpecRegs[uiPort];
      ulRange0Size    = sizeof(NX51_XPEC_AREA_T);

      /* PRAM Area */
      pulRange1Dpm    = s_apulTpecPramArea[uiPort];
      ulRange1Start   = s_aulNX6TpecPram[uiPort];
      ulRange1Size    = NX6_XPEC_DWORD_RAMSIZE * sizeof(uint32_t);

      /* DRAM Area */
      pulRange2Dpm    = s_apulXpecDramArea[uiPort];
      ulRange2Start   = s_aulNX6XpecDram[uiPort] + NX6_XPEC_DWORD_RAMSIZE * sizeof(uint32_t);
      ulRange2Size    = NX6_XPEC_DWORD_RAMSIZE * sizeof(uint32_t);
      break;

    case NX6_XC_TYPE_RPU:
      pulRamStart     = s_aptRpuPramArea[uiPort];
      pulRamEnd       = s_aptRpuPramAreaEnd[uiPort]+1;

      /* Register Area */
      pulRange0Dpm    = (volatile uint32_t*)s_aptXmacArea[uiPort];
      ulRange0Start   = s_aulNX6XmacRegs[uiPort];
      ulRange0Size    = sizeof(NX51_XMAC_AREA_T);

      /* PRAM Area */
      pulRange1Dpm    = s_aptRpuPramArea[uiPort];
      ulRange1Start   = s_aulNX6RpuPram[uiPort];
    ...
    case NX6_XC_TYPE_TPU:
      pulRamStart     = s_aptTpuPramArea[uiPort];
      ...
      /* PRAM Area */
      pulRange1Dpm    = s_aptTpuPramArea[uiPort];
      ulRange1Start   = s_aulNX6TpuPram[uiPort];
...
*   -1 on erroneous                                                          */
/*****************************************************************************/
int NX6_XC_Start( unsigned int uiPort,
                  void*        pvUser )
{
...
int NX6_XC_StartUnits(unsigned int uiPort, unsigned int uiUnitVec, void* pvUser)
{
  ...
  if( 0 != (uiUnitVec & MSK_NX6_XC_TYPE_RPU) )
  {
    ulStart |= MSK_NX51_xc_start_stop_ctrl_start_rpu0;
  }

  if( 0 != (uiUnitVec & MSK_NX6_XC_TYPE_TPU) )
  {
    ulStart |= MSK_NX51_xc_start_stop_ctrl_start_tpu0;
  }

  if( 0 != (uiUnitVec & MSK_NX6_XC_TYPE_RPEC) )
  {
    ulStart |= MSK_NX51_xc_start_stop_ctrl_start_rpec0;
  }

  if( 0 != (uiUnitVec & MSK_NX6_XC_TYPE_TPEC) )
  {
    ulStart |= MSK_NX51_xc_start_stop_ctrl_start_tpec0;
  }

  NX_WRITE32(s_ptXcStartStop->ulXc_start_stop_ctrl, ulStart << (4 * uiPort));
