...
/* netX10 specific settings */
#if defined(HAL_ETHMAC_TARGET_NX10)
#include "hal_resources_defines_netx10.h"
/* XC includes */
#include "xpec_ethmac0.h"
#include "rpu_ethmac0.h"
#include "tpu_ethmac0.h"
#define NXT(id)                  ETHMAC_CONCAT(NX10_,id)
#define MSK_NXT(id)              ETHMAC_CONCAT(MSK_NX10_,id)
#define ETHMAC_PORTS             1
                               ...
                               uint32_t *pulIrq )
{
  ...
  /* get IRQ events */
  *pulIrq = NX_READ32(s_ptXpecIrqs->ulIrq_xpec0) & 0xffff;
                                   ...
                                   uint32_t ulConfirmIrqMask )
{
  ...
  NX_WRITE32(s_ptXpecIrqs->ulIrq_xpec0, ulConfirmIrqMask & 0xffff);
                                   ...
                                   void* pvUser )
{
  ...
  s_aptXpecDRam[uiPort] = (ETHMAC_XPEC_DPM*) s_pulXpecDramArea[uiPort];
                                    ...
                                    bool fFdx )
{
  ...
  /* set request */
  NX_WRITE32(s_ptXpecIrqs->ulIrq_xpec0, MSK_ETHMAC_ARM2XPEC_INTERRUPTS_LINK_MODE_UPDATE_REQ | ulVal);

  /* wait for xPEC confirmed request */
  while( NX_READ32(s_ptXpecIrqs->ulIrq_xpec0 & MSK_ETHMAC_ARM2XPEC_INTERRUPTS_LINK_MODE_UPDATE_REQ) ) {}
