...
int HALDEF(IFlash_SetRedundancyPage)(unsigned int uiInst, unsigned int uiRedSel, unsigned int uiPage)
{
  if( uiRedSel > 1 )
  {
...
int HALDEF(IFlash_BlockSelect)(unsigned int uiInst, IFLASH_BLOCK_E eBlock)
{
  uint32_t ulVal = 0;
...
int HALDEF(IFlash_Program)(unsigned int uiInst, uint32_t ulXAdr, uint32_t ulYAdr, const uint32_t* pulData, unsigned int uiLen)
{
  unsigned int uiCell;
...
int HALDEF(IFlash_ProgramMan)(unsigned int uiInst, uint32_t ulXAdr, uint32_t ulYAdr, const uint32_t* pulData, unsigned int uiLen)
{
  unsigned int uiCell;
  const uint32_t* pulSrc = pulData;
...
int HALDEF(IFlash_StartEraseMan)(unsigned int uiInst, unsigned int uiPage)
{
  uint32_t ulSignals = HW_MSK(iflash_signals_cfg_xe);
...
int HALDEF(IFlash_StopEraseMan)(unsigned int uiInst)
{
...
int HALDEF(IFlash_StartErase)(unsigned int uiInst, unsigned int uiPage)
{
  /* setup programming */
  if( uiPage == IFLASH_PAGE_ALL )
  {
...
int HALDEF(IFlash_StopErase)(unsigned int uiInst)
{
...
int HALDEF(IFlash_EnterTestmode)(unsigned int uiInst, unsigned int uiCode)
{
...
int HALDEF(IFlash_ExitTestmode)(unsigned int uiInst)
{
  HW_WR32(s_aptIFlash[uiInst]->ulIflash_special_cfg, HW_MSK(iflash_special_cfg_tmr));
  HW_WR32(s_aptIFlash[uiInst]->ulIflash_mode_cfg, IFLASH_MODE_READ);
...
int HALDEF(IFlash_SetProtection)(unsigned int uiInst, uint32_t ulVal)
{
...
#if defined(CPU_IS_ARM)
int HALDEF(IFlash_SetGlobalTiming)(unsigned int uiMultiplier, unsigned int uiDivisor)
{
  uint32_t ulTmp;
