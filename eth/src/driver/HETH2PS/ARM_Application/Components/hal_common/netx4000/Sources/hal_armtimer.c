...
{
  if( uiInst>=ARMTIMER_CHANNELS )
    return ARMTIMER_ERR_INVAL_PARAM;

  s_ptArmTimer->aulArm_timer_config_timer[uiInst]  &=  ~HW_MSK(arm_timer_config_timer0_systime_config);
  s_ptArmTimer->aulArm_timer_config_timer[uiInst]  |=  uiSystimeInst<<HW_SRT(arm_timer_config_timer0_systime_config);
...
ARMTIMER_RESULT_E ArmTimer_Start( unsigned int uiInst, ARMTIMER_CFG_MODE_E eMode, uint32_t ulVal )
{
  ...
  ulSystimeConfig = s_ptArmTimer->aulArm_timer_config_timer[uiInst]
                  & HW_MSK(arm_timer_config_timer0_systime_config);

  switch( eMode )
  {
    case ARMTIMER_CFG_MODE_STOP_AT_0:
      s_ptArmTimer->aulArm_timer_config_timer[uiInst]  =  ulSystimeConfig | ((uint32_t)eMode << HW_SRT(arm_timer_config_timer0_mode));
      s_ptArmTimer->aulArm_timer_timer[uiInst]         =  ulVal;
    break;

    case ARMTIMER_CFG_MODE_PRELOAD_AT_0:
      s_ptArmTimer->aulArm_timer_config_timer[uiInst]  =  ulSystimeConfig | ((uint32_t)eMode << HW_SRT(arm_timer_config_timer0_mode));
      s_ptArmTimer->aulArm_timer_preload_timer[uiInst] =  ulVal;
      s_ptArmTimer->aulArm_timer_timer[uiInst]         =  ulVal;
    break;
    ...
    default: eRslt = ARMTIMER_ERR_INVAL_PARAM;
    break;
  }
...
ARMTIMER_RESULT_E ArmTimer_Stop( unsigned int uiInst )
{
  ...
  /* disable interrupt */
  s_ptArmTimer->ulArm_timer_irq_msk_reset = HW_MSK(arm_timer_irq_msk_reset_timer0_irq) << uiInst;

  /* reset timer registers */
  s_ptArmTimer->aulArm_timer_config_timer[uiInst] &=  ~HW_MSK(arm_timer_config_timer0_mode); /* don't touch systime config */
  s_ptArmTimer->aulArm_timer_preload_timer[uiInst] =  0;
  s_ptArmTimer->aulArm_timer_timer[uiInst]         =  0;
  s_ptArmTimer->aulArm_timer_timer[uiInst]         =  0;

  /* confirm pending interrupt */
  s_ptArmTimer->ulArm_timer_irq_raw = HW_MSK(arm_timer_irq_raw_timer0_irq) << uiInst;
...
{
  s_ptArmTimer->aulArm_timer_config_timer[uiInst]  =  ARMTIMER_CFG_MODE_STOP_AT_0 << HW_SRT(arm_timer_config_timer0_mode);
  s_ptArmTimer->aulArm_timer_timer[uiInst]         =  ulTimeout10Ns;

  /* poll timer finished */
  while ( s_ptArmTimer->aulArm_timer_timer[uiInst] != 0 );
}
...
*   ARMTIMER
* \params
*   uiSysimeInst              [in]   Selected instance
* \return
*                                                                            */
/*****************************************************************************/
void ArmTimer_SetSystimeConfig ( unsigned int uiSysimeInst )
{
  s_ptArmTimer->ulArm_timer_systime_config = uiSysimeInst;
}
...
{
  return s_ptArmTimer->aulArm_timer_timer[uiInst];
}
