...
#if defined(HAL_ETHMAC_TARGET_NX4000)
#include "hal_resources_defines_netx4000.h"
/* XC includes */
#include "rpec_ethmac_rpec0.h"
#include "rpec_ethmac_rpec1.h"
#include "tpec_ethmac_tpec0.h"
#include "tpec_ethmac_tpec1.h"
#include "rpu_ethmac0.h"
#include "rpu_ethmac1.h"
#include "tpu_ethmac0.h"
#include "tpu_ethmac1.h"
#include "rpec_ethmac_rpec2.h"
#include "rpec_ethmac_rpec3.h"
#include "tpec_ethmac_tpec2.h"
#include "tpec_ethmac_tpec3.h"
#include "rpu_ethmac2.h"
#include "rpu_ethmac3.h"
#include "tpu_ethmac2.h"
#include "tpu_ethmac3.h"
#define NXT(id)                  ETHMAC_CONCAT(NX4000_,id)
#define MSK_NXT(id)              ETHMAC_CONCAT(MSK_NX4000_,id)
#define SRT_NXT(id)              ETHMAC_CONCAT(SRT_NX4000_,id)
#define ETHMAC_PORTS             4

/* netX90 specific settings */
#elif defined(HAL_ETHMAC_TARGET_NX90)
...
#include "tpu_ethmac0.h"
#include "tpu_ethmac1.h"
#define NXT(id)                  ETHMAC_CONCAT(NX90_,id)
#define MSK_NXT(id)              ETHMAC_CONCAT(MSK_NX90_,id)
#define SRT_NXT(id)              ETHMAC_CONCAT(SRT_NX90_,id)
...
static const uint32_t* const paulRPecCodes[]=
{
  XcCode_rpec_ethmac_rpec0,
  XcCode_rpec_ethmac_rpec1,
#if ETHMAC_PORTS > 2
  XcCode_rpec_ethmac_rpec2,
  XcCode_rpec_ethmac_rpec3,
#endif
};

static const uint32_t* const paulTPecCodes[]=
{
  XcCode_tpec_ethmac_tpec0,
  XcCode_tpec_ethmac_tpec1,
#if ETHMAC_PORTS > 2
  XcCode_tpec_ethmac_tpec2,
  XcCode_tpec_ethmac_tpec3,
#endif
};

static const uint32_t* const paulxMacRpuCodes[]=
{
  XcCode_rpu_ethmac0,
  XcCode_rpu_ethmac1,
#if ETHMAC_PORTS > 2
...
static const uint32_t* const paulxMacTpuCodes[]=
{
  XcCode_tpu_ethmac0,
  XcCode_tpu_ethmac1,
#if ETHMAC_PORTS > 2
...
static void initFifoUnit( unsigned int uiPort )
{
  ...
  NX_WRITE32(s_aptPFifo[uiXc]->ulPfifo_reset, NX_READ32(s_aptPFifo[uiXc]->ulPfifo_reset) | ulFifoMsk);
  ...
  {
    NX_WRITE32(s_aptPFifo[uiXc]->aulPfifo_border[uiIdx], (uiIdx * FIFO_ENTRIES) + FIFO_ENTRIES - 1);
  }
  ...
  for( uiIdx = 1; uiIdx <= uiEmptyPtrCnt; uiIdx++ )
  {
    ...
    NX_WRITE32(s_aptPFifo[uiXc]->aulPfifo[uiFifoStart + ETHERNET_FIFO_EMPTY], ulFifoPtr);
  }
}
                                     ...
                                     const ETHERNET_MAC_ADDR_T tMacAddr)
{
  ...
  uiOfs = tMacAddr[5] >> 1;
  uiBit = (((tMacAddr[5] & 1U) << 4) | (tMacAddr[4] & 0xfU));
  ulMsk = 1UL << uiBit;
                                 ...
                                 void** phFrame )
{
  ...
  if( NX_READ32(s_aptPFifo[uiXc]->aulPfifo_fill_level[uiFifo]) < 2)
    return ETH_ERR_FIFO_EMPTY;

  /* retrieve the FIFO element */
  ulFifoPtr = NX_READ32(s_aptPFifo[uiXc]->aulPfifo[uiFifo]);
                                            ...
                                            uint32_t *pulCnfFillLevel )
{
  ...
  *pulCnfFillLevel = NX_READ32(s_aptPFifo[uiXc]->aulPfifo_fill_level[uiFifo]);
                             ...
                             unsigned int uHighPriority )
{
  ...
  NX_WRITE32(s_aptPFifo[uiXc]->aulPfifo[uiFifo], ulFifoPtr);
                                    ...
                                    ETHMAC_COUNTERS_T* ptCounters )
{
  ...
  ptCounters->ulETHMAC_OUT_FRAMES_OKAY                   = NX_READ32( s_aptXpecDRam[uiPort]->tETHMAC_STATUS_AREA_BASE.ulETHMAC_OUT_FRAMES_OKAY );
  ...
  ptCounters->ulETHMAC_IN_FRAMES_OKAY                    = NX_READ32( s_aptXpecDRam[uiPort]->tETHMAC_STATUS_AREA_BASE.ulETHMAC_IN_FRAMES_OKAY );
                                         ...
                                         uint32_t *pulFillLevel )
{
  ...
  *pulFillLevel = NX_READ32(s_aptPFifo[uiXc]->aulPfifo_fill_level[uiFifo]);
                               ...
                               uint32_t *pulIrq )
{
  ...
  /* get IRQ events */
  *pulIrq = NX_READ32(s_ptXpecIrqRegs->aulIrq_xpec[uiPort]) & 0xffff;
                                   ...
                                   uint32_t ulConfirmIrqMask )
{
  ...
  NX_WRITE32(s_ptXpecIrqRegs->aulIrq_xpec[uiPort], ulConfirmIrqMask & 0xffff);
                                   ...
                                   void* pvUser )
{
  ...
  s_aptXpecDRam[uiPort] = (ETHMAC_XPEC_DPM*) s_apulXpecDramArea[uiPort];

  /* reset the hardware block */
  if( pfnXcReset(uiXc, uiXpec, pvUser) != 0 )
  ...
  if(  ( 0 != pfnXcLoad(uiXc, uiXpec, NX_XC_TYPE_RPU,  paulxMacRpuCodes[uiPort], pvUser) )
     ||( 0 != pfnXcLoad(uiXc, uiXpec, NX_XC_TYPE_TPU,  paulxMacTpuCodes[uiPort], pvUser) )
     ||( 0 != pfnXcLoad(uiXc, uiXpec, NX_XC_TYPE_RPEC, paulRPecCodes[uiPort], pvUser) )
     ||( 0 != pfnXcLoad(uiXc, uiXpec, NX_XC_TYPE_TPEC, paulTPecCodes[uiPort], pvUser) )
    )
    return ETH_ERR_INIT_FAILED;
  ...
  NX_WRITE32(s_aptXpecDRam[uiPort]->tETHMAC_CONFIG_AREA_BASE.ulETHMAC_SYSTIME_BORDER_COPY_PLUS1, NX_READ32(s_aptSystime[SYSTIME_INST]->ulSystime_border) + 1);

  /* configure PHY LED unit */
  ulActLedFlashPeriod = ulActLedFlashPeriod / 1000000; /* convert from 10ns->10ms */
  switch( ePhyLedCfg ) {
    case ETH_PHY_LED_STATIC:
      NX_WRITE32(s_aptPhyCtrl[uiPort]->ulInt_phy_ctrl_led,
          NX_INT_PHYCTRL_LED_MODE_STATIC << SRT_NXT(int_phy_ctrl_led_mode) );
    break;

    case ETH_PHY_LED_BLINK:
      NX_WRITE32(s_aptPhyCtrl[uiPort]->ulInt_phy_ctrl_led,
          (NX_INT_PHYCTRL_LED_MODE_FLASHING << SRT_NXT(int_phy_ctrl_led_mode))
               | ((ulActLedFlashPeriod - 1) << SRT_NXT(int_phy_ctrl_led_interval)) );
    break;

    case ETH_PHY_LED_SINGLE:
      NX_WRITE32(s_aptPhyCtrl[uiPort]->ulInt_phy_ctrl_led,
          (NX_INT_PHYCTRL_LED_MODE_COMBINED << SRT_NXT(int_phy_ctrl_led_mode))
               | ((ulActLedFlashPeriod - 1) << SRT_NXT(int_phy_ctrl_led_interval)) );
    break;

    case ETH_PHY_LED_OFF:
      NX_WRITE32(s_aptPhyCtrl[uiPort]->ulInt_phy_ctrl_led, 0);
    break;
                              ...
                              void* pvUser )
{
  ...
  /* start XC port */
  if( pfnXcStart(uiXc, uiXpec, pvUser) != 0 )
    return ETH_ERR_INIT_FAILED;
                                    ...
                                    bool fFdx )
{
  ...
  /* set request */
  NX_WRITE32(s_ptXpecIrqRegs->aulIrq_xpec[uiPort], MSK_ETHMAC_ARM2XPEC_INTERRUPTS_LINK_MODE_UPDATE_REQ | ulVal);

  /* wait for xPEC confirmed request */
  while( NX_READ32(s_ptXpecIrqRegs->aulIrq_xpec[uiPort] & MSK_ETHMAC_ARM2XPEC_INTERRUPTS_LINK_MODE_UPDATE_REQ) ) {}
...
{
  ETHERNET_RESULT eRslt = ETH_OKAY;
  uint32_t ulVal;
  unsigned int uiMiiOutputPhase = 0;
  unsigned int uiTxCorrBase = 15;
  ...
  case 1:
    /* external MII */
    /* Note: In netx4000full one additional output-flipflop is inserted in each path to external MII (to pads XM??_TX{D?,EN,ER}). */
  ...
  default:
    eRslt = ETH_ERR_INVALID_PARAMETER;
    break;
  }

  /* set output phase */
  ulVal  = NX_READ32(s_aptXmacArea[uiPort]->ulXmac_config_tx_nibble_fifo);
  ...
  NX_WRITE32( s_aptXmacArea[uiPort]->ulXmac_config_tx_nibble_fifo, ulVal );

  /* update correction base values */
  NX_WRITE32(s_aptXpecDRam[uiPort]->tETHMAC_CONFIG_AREA_BASE.ulETHMAC_TS_COR_BASE_TX, uiTxCorrBase);
