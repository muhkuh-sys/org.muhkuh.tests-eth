...
class sizereport(Task.Task):
    ...
    _symbols_cache_by_objectfile = None
    ...
    @property
    def symbols_by_objectfile(self):
        if self._symbols_cache_by_objectfile is None:
            _symbols_cache_by_objectfile = {}

            for x in self.symbols_by_name.values():
                try:
                    _symbols_cache_by_objectfile.setdefault(x.objectfile, []).append(x)
                except AttributeError:
                    pass

            self._symbols_cache_by_objectfile = _symbols_cache_by_objectfile

        return self._symbols_cache_by_objectfile
    ...
    def addSymbol(self, name):
        ...
        self._symbols_cache_by_objectfile = None
    ...
    def run(self):
        ...
        items = list(x() for x in (self.generateHTMLIntro,
                                   ...
                                   self.generateHTMLObjectFileByName))
    ...
    def parseMapFile(self, path):
        ...
        def mapping_handler(self, line):
            ...
            if line.lower().startswith("load"):
                pass
            elif line.lower().startswith("start group"):
                pass
            elif line.lower().startswith("end group"):
                pass
                ...
                if module_section is not None:
                    ...
                    if module_section not in (".ARM.attributes",) and len(l) >= 2:
                        section, name = l

                        try:
                            symbol = self.symbols_by_name[name]
                        except KeyError:
                            # This symbol does not exist in symbols list, add it and fill with info
                            name = name +" ({0})".format(state.address)
                            symbol = self.addSymbol(name)
                            symbol.size  = int(state.size,16)
                            symbol.value = int(state.address,16)
                            symbol.section = section

                        # Add symbol informations about module (archive or objectfile) and objectfile.
                        if state.archive:
                            symbol.module = os.path.basename(state.archive)
                            if state.archive[-2:] == ".o":
                                symbol.objectfile = os.path.basename(state.archive)
                            elif state.archive_member:
                                symbol.objectfile = os.path.basename(state.archive_member)
    ...
    def generateHTMLSecByName(self):
        ...
        content.append('<h2 id="secbyname">Symbols by Section and Name</h2>')
    ...
    def generateHTMLModulesByName(self):
        ...
        content.append('<h2 id="modbyname">Symbols by Module and Name</h2>')

        content_summ.append('<h3 id="modbyname_summary">Module summary</h3>')
        content_summ.append('<table>\n')
        content_summ.append('<tr><th>Module Name</th><th>Size text/rodata</th><th>Size data/bss</th><th>Size xip</th></tr>')

        sum_data_summ, sum_text_summ, sum_xip_summ = 0, 0, 0
        ...
        for module, symbols in sorted(self.symbols_by_module.items(), key = lambda x: x[0]):
            ...
            if symbols:
                ...
                sum_data_summ += sum_data
                sum_text_summ += sum_text
                sum_xip_summ  += sum_xip
                ...
                content_summ.append('<tr><td>%s</td><td class="rightalign">%d</td><td class="rightalign">%d</td><td class="rightalign">%d</td></tr>' % (module, sum_text, sum_data, sum_xip))

        content_summ.append('<tr><th>SUM</th><th class="rightalign">%d</th><th class="rightalign">%d</th><th class="rightalign">%d</th></tr>' % (sum_text_summ, sum_data_summ, sum_xip_summ))
        content_summ.append('</table>')

        content.extend(content_summ)
        toc.append('<li><a href="#modbyname_summary">Module summary</a></li>')

        toc.append('</ol>')

        return (toc,content)

    def generateHTMLObjectFileByName(self):
        toc     = []
        content = []
        content_summ = []
        ...
        # generate section by name toc
        toc.append('<a href="#objfilebyname">Symbols by ObjectFile and Name</a><ol>')
        content.append('<h2 id="objfilebyname">Symbols by ObjectFile and Name</h2>')

        content_summ.append('<h3 id="objfilebyname_summary">ObjectFile summary</h3>')
        content_summ.append('<table>\n')
        content_summ.append('<tr><th>ObjectFile Name</th><th>Size text/rodata</th><th>Size data/bss</th><th>Size xip</th></tr>')

        sum_data_summ, sum_text_summ, sum_xip_summ = 0, 0, 0

        for objectfile, symbols in sorted(self.symbols_by_objectfile.items(), key = lambda x: x[0]):
            symbols = tuple(x for x in symbols if getattr(x,'size', 0))

            if symbols:
                anchor = objectfile.translate(string.maketrans(".","_"))

                toc.append('<li><a href="#objfilebyname_%s">ObjectFile &quot;%s&quot</a></li>' % (anchor, objectfile))
                content.append('<h3 id="objfilebyname_%s">ObjectFile &quot;%s&quot</h3>' % (anchor, objectfile))
                ...
                content_summ.append('<tr><td>%s</td><td class="rightalign">%d</td><td class="rightalign">%d</td><td class="rightalign">%d</td></tr>' % (objectfile, sum_text, sum_data, sum_xip))

        content_summ.append('<tr><th>SUM</th><th class="rightalign">%d</th><th class="rightalign">%d</th><th class="rightalign">%d</th></tr>' % (sum_text_summ, sum_data_summ, sum_xip_summ))
        content_summ.append('</table>')

        content.extend(content_summ)
        toc.append('<li><a href="#objfilebyname_summary">ObjectFile summary</a></li>')
