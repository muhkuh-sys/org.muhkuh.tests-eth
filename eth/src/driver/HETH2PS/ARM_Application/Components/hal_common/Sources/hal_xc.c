...
*   XC
* \params
*   uiXcInst        [in]  XC instance
*   uiXcPort        [in]  XC port number
*   pvUser          [in]  User specific parameter
* \return
*   0 on success
*   -1 on erroneous                                                          */
/*****************************************************************************/
int XC_Reset( unsigned int uiXcInst, unsigned int uiXcPort, void* pvUser )
{
  ...
  unsigned int uIdx;
  unsigned int uiPort = uiXcInst * 2 + uiXcPort;
  ...
  ptRpec->aulStatcfg[uiXcPort] = 0xffff0000;
  ...
  if( XC_Load( uiXcInst, uiXcPort, XC_TYPE_RPU, paulxMacRpuCodes[uiPort], pvUser) != 0) {
    return -2;
  }
  if( XC_Load( uiXcInst, uiXcPort, XC_TYPE_TPU, paulxMacTpuCodes[uiPort], pvUser) != 0) {
    return -3;
  }
  ...
  s_ptXcStartStop->ulXc_start_stop_ctrl = (HW_MSK(xc_start_stop_ctrl_xc0_start_rpu0) |
                                           HW_MSK(xc_start_stop_ctrl_xc0_start_tpu0)) << (4 * uiPort);
  ...
  for (uIdx = 0; uIdx < 10; uIdx++)
  {
    s_ptXcStartStop->ulXc_start_stop_ctrl = (HW_MSK(xc_start_stop_ctrl_xc0_start_rpec0) |
                                             HW_MSK(xc_start_stop_ctrl_xc0_start_tpec0)) << (4 * uiPort);
  }

  s_ptXcStartStop->ulXc_start_stop_ctrl = (HW_MSK(xc_start_stop_ctrl_xc0_stop_rpec0) |
                                           HW_MSK(xc_start_stop_ctrl_xc0_stop_tpec0)) << (4 * uiPort);
  ptRpec->ulXpec_pc     = 0x7ff;                                 /* Reset the Program Counter to 0x7ff */
  ptTpec->ulXpec_pc     = 0x7ff;                                 /* Reset the Program Counter to 0x7ff */
  ...
  for( uIdx = (4 * uiXcPort); uIdx < (4 * uiXcPort + 4); ++uIdx )
    ptRpec->aulXpec_sr[uIdx] = 0;
  /* Reset SR8-11 for XPEC0, SR12-15 for XPEC1 */
  for( uIdx = (8 + 4 * uiXcPort); (uIdx < 12 + 4 * uiXcPort); ++uIdx )
    ptRpec->aulXpec_sr[uIdx] = 0;
  ...
  s_ptXcStartStop->ulXc_start_stop_ctrl = (HW_MSK(xc_start_stop_ctrl_xc0_stop_rpu0) |
                                           HW_MSK(xc_start_stop_ctrl_xc0_stop_tpu0)) << (4 * uiPort);
...
*   -1 on erroneous                                                          */
/*****************************************************************************/
int XC_Load(unsigned int uiXcInst, unsigned int uiXcPort, XC_TYPE_E eXcType, const uint32_t* pulXcPrg, void* pvUser)
{
  ...
  switch( eXcType )
  {
    case XC_TYPE_RPEC: pulRamStart     = s_apulRpecPramArea[uiPort];
                       pulRamEnd       = s_apulRpecPramAreaEnd[uiPort]+1;
                       break;
    case XC_TYPE_TPEC: pulRamStart     = s_apulTpecPramArea[uiPort];
                       pulRamEnd       = s_apulTpecPramAreaEnd[uiPort]+1;
                       break;
    case XC_TYPE_RPU:  pulRamStart     = s_aptRpuPramArea[uiPort];
                       pulRamEnd       = s_aptRpuPramAreaEnd[uiPort]+1;
                       break;
    case XC_TYPE_TPU:  pulRamStart     = s_aptTpuPramArea[uiPort];
...
*   -1 on erroneous                                                          */
/*****************************************************************************/
int XC_Start(unsigned int uiXcInst, unsigned int uiXcPort, void* pvUser)
{
...
*   -1 on erroneous                                                          */
/*****************************************************************************/
int XC_StartUnits(unsigned int uiXcInst, unsigned int uiXcPort, unsigned int uiUnitVec, void* pvUser)
{
...
*   -1 on erroneous                                                          */
/*****************************************************************************/
int XC_Stop(unsigned int uiXcInst, unsigned int uiXcPort, void* pvUser)
{
  unsigned int uiPort = uiXcInst * 2 + uiXcPort;
...
*   -1 on erroneous                                         */
/*****************************************************************************/
int XC_GetReg ( unsigned int uiXcInst, unsigned int uiXcPort, XC_TYPE_E eXcType, XC_REG_E eReg, uint32_t * pulValue )
{
  int iRslt = 0;
  ...
  switch( eXcType )
  {
    ...
    case XC_TYPE_RPEC:
    case XC_TYPE_TPEC:
      ptXpec = ( eXcType == XC_TYPE_RPEC ) ? s_aptRpecRegArea[uiPort]
                                           : s_aptTpecRegArea[uiPort];
      switch( eReg )
      {
...
*   -1 on erroneous                                         */
/*****************************************************************************/
int XC_SetReg ( unsigned int uiXcInst, unsigned int uiXcPort, XC_TYPE_E eXcType, XC_REG_E eReg, uint32_t ulValue )
{
  int iRslt = 0;
  HW_TYPE(XMAC_AREA_T)* ptXmac = NULL;
  HW_TYPE(XPEC_AREA_T)* ptXpec = NULL;
...
* \params
*   uiPort              [in]  XC port number
*   uiOffset            [in]  DRAM Offset
*   ulVal               [in]  Value
* \return
*   0 on success
*   -1 on erroneous                                                          */
/*****************************************************************************/
int XC_WrXpecDramDw( unsigned int uiXcInst, unsigned int uiXcPort, unsigned int uiOffset, uint32_t ulVal )
{
  volatile uint32_t* pulXpecDram;
  unsigned int uDwOffset = uiOffset/sizeof(uint32_t);
  unsigned int uiPort = uiXcInst * 2 + uiXcPort;
  ...
  pulXpecDram = s_apulXpecDramArea[uiPort];
...
*   Read value                                                               */
/*****************************************************************************/
uint32_t XC_RdXpecDramDw( unsigned int uiXcInst, unsigned int uiXcPort, unsigned int uiOffset )
{
  volatile uint32_t* pulXpecDram;
  unsigned int uDwOffset = uiOffset/sizeof(uint32_t);
  unsigned int uiPort = uiXcInst * 2 + uiXcPort;
...
*   -1 on erroneous                                                          */
/*****************************************************************************/
int XC_WrRpecDramDw( unsigned int uiXcInst, unsigned int uiXcPort, unsigned int uiOffset, uint32_t ulVal )
{
  ...
  pulRpecDram = s_apulRpecDramArea[uiPort];
...
*   Read value                                                               */
/*****************************************************************************/
uint32_t XC_RdRpecDramDw( unsigned int uiXcInst, unsigned int uiXcPort, unsigned int uiOffset )
{
  volatile uint32_t* pulRpecDram;
  unsigned int uiDwOffset = uiOffset/sizeof(uint32_t);
  unsigned int uiPort = uiXcInst * 2 + uiXcPort;

  /* check instance number and Offset */
  if( uiPort >= NUM_XPECS ) return 0;
  if( uiDwOffset >= XPEC_DWORD_RAMSIZE ) return 0;

  pulRpecDram = s_apulRpecDramArea[uiPort];

  /* Read value */
  return pulRpecDram[uiDwOffset];
}
...
*   int                                                                      */
/*****************************************************************************/
int XC_SystimeConfigSet( unsigned int uiXcInst, unsigned int uiSystimeInstNo )
{
  int iRslt = 0;
  uint32_t ulVal = s_ptXcSystimeConfig->ulXc_systime_config;

  /* Check the instance number */
  if( uiSystimeInstNo >= 4 ) return -1;
  ...
  switch( uiXcInst )
  {
    case 0:
      ...
      ulVal |= (uint32_t)uiSystimeInstNo << HW_SRT(xc_systime_config_xc0_systime_config);
    ...
    case 1:
      ...
      ulVal |= (uint32_t)uiSystimeInstNo << HW_SRT(xc_systime_config_xc1_systime_config);
      s_ptXcSystimeConfig->ulXc_systime_config = ulVal;
    break;
#endif
    default:
...
*   -1 on erroneous                                                          */
/*****************************************************************************/
int XC_LoadChannel ( unsigned int uiXcInst, unsigned int uiXcPort, XC_PRG_T* ptPrg, void* pvUser )
{
  int iError;

  iError = XC_Load(uiXcInst, uiXcPort, XC_TYPE_RPEC, ptPrg->pulRpecPrg, pvUser);
  if( iError ) return iError;
  iError = XC_Load(uiXcInst, uiXcPort, XC_TYPE_TPEC, ptPrg->pulTpecPrg, pvUser);
  if( iError ) return iError + 5;
  iError = XC_Load(uiXcInst, uiXcPort, XC_TYPE_RPU, ptPrg->pulRpuPrg, pvUser);
  if( iError ) return iError + 10;
  iError = XC_Load(uiXcInst, uiXcPort, XC_TYPE_TPU, ptPrg->pulTpuPrg, pvUser);
...
*                                                                            */
/*****************************************************************************/
void XC_SetFbClk(unsigned int uiXcInst, unsigned int uXpec, uint32_t ulFbClkRateMulAdd, uint32_t ulFbClkDiv)
{
  if( uXpec<2 )
  {
#if defined(__NETX4000)
