...
/*****************************************************************************/
ARMTIMER_RESULT_E ArmTimer_ConfigureTimerSystime( unsigned int uiInst, unsigned int uSystimeInst )
{
  if( uiInst >= ARMTIMER_CHANNELS )
    return ARMTIMER_ERR_INVAL_PARAM;

  s_ptArmTimer->aulTimer_config_timer[uiInst]  &=  ~HW_MSK(timer_config_timer0_systime_config);
  s_ptArmTimer->aulTimer_config_timer[uiInst]  |=  uSystimeInst<<HW_SRT(timer_config_timer0_systime_config);
...
ARMTIMER_RESULT_E ArmTimer_Start( unsigned int uiInst, ARMTIMER_CFG_MODE_E eMode, uint32_t ulVal )
{
  ...
  ulSystimeConfig = s_ptArmTimer->aulTimer_config_timer[uiInst]
                  & HW_MSK(timer_config_timer0_systime_config);

  switch( eMode )
  {
    case ARMTIMER_CFG_MODE_STOP_AT_0:
      s_ptArmTimer->aulTimer_config_timer[uiInst]  = ulSystimeConfig | ((uint32_t)eMode << HW_SRT(timer_config_timer0_mode));
      s_ptArmTimer->aulTimer_timer[uiInst]         = ulVal;
    break;

    case ARMTIMER_CFG_MODE_PRELOAD_AT_0:
      s_ptArmTimer->aulTimer_config_timer[uiInst]  = ulSystimeConfig | ((uint32_t)eMode << HW_SRT(timer_config_timer0_mode));
      s_ptArmTimer->aulTimer_preload_timer[uiInst] = ulVal;
      s_ptArmTimer->aulTimer_timer[uiInst]         = ulVal;
    break;
...
ARMTIMER_RESULT_E ArmTimer_Stop( unsigned int uiInst )
{
  ...
  /* disable interrupt */
  s_ptArmTimer->ulTimer_irq_msk_reset = HW_MSK(timer_irq_msk_reset_timer0_irq) << uiInst;

  /* reset timer registers */
  s_ptArmTimer->aulTimer_config_timer[uiInst] &= ~HW_MSK(timer_config_timer0_mode); /* don't touch systime config */
  s_ptArmTimer->aulTimer_preload_timer[uiInst] = 0;
  s_ptArmTimer->aulTimer_timer[uiInst]         = 0;
  s_ptArmTimer->aulTimer_timer[uiInst]         = 0;

  /* confirm pending interrupt */
  s_ptArmTimer->ulTimer_irq_raw = HW_MSK(timer_irq_raw_timer0_irq) << uiInst;
...
{
  s_ptArmTimer->aulTimer_config_timer[uiInst]  =  ARMTIMER_CFG_MODE_STOP_AT_0 << HW_SRT(timer_config_timer0_mode);
  s_ptArmTimer->aulTimer_timer[uiInst]         =  ulTimeout10Ns;

  /* poll timer finished */
  while ( s_ptArmTimer->aulTimer_timer[uiInst] != 0 );
}
...
*                                                                            */
/*****************************************************************************/
void ArmTimer_SetSystimeConfig ( unsigned int uSysimeInst )
{
...
{
  return s_ptArmTimer->aulTimer_timer[uiInst];
}
